...
if __name__ == '__main__':
    img_dir = "./images/"
    align_dir = './record/align/'
    model = MTCNN()

    for image_path in glob(img_dir + os.sep + "*jpg"):
        img_name = image_path.split('.')[0]
        img = Image.open(image_path)
        boxes, faces = model.align_multi(img, limit=None, min_face_size=15.0)
        for index, face in enumerate(faces):
            face.save(align_dir + img_name + '_' + '.jpg')
