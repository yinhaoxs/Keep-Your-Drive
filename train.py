...
from model import dataset, model, train_loss, valid_loss, evalute
from data import splitpairs, dataread
...
parse.add_argument('--root_path', type=str, default='./imageset/image/', help='origin image to train the kyd_net')
...
parse.add_argument('--log_save_path', type=str, default='./record/log/', help='log save for train and valid')
parse.add_argument('--model_save_path', type=str, default='./record/checkpoint/', help='model save for strain and valid')
...
parse.add_argument('--data_path', type=str, default='./imageset/label/', help='the path for csv files')
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epoches+1):
        ...
        KYD_train_output_lists = evalute.evalute(KYD_train_loader, model, args.device)
        KYD_valid_output_lists = evalute.evalute(KYD_valid_loader, model, args.device)
        LFW_train_output_lists = evalute.evalute(LFW_train_loader, model, args.device)
        LFW_valid_output_lists = evalute.evalute(LFW_valid_loader, model, args.device)
