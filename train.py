...
from models import dataset, models, trainloss, validloss, compute_gini
from utils import gini, splitpairs, data
...
parse.add_argument('--model_save_path', type=str, default='./checkpoints/', help='model save for strain and valid')
...
## 计算gini的准备文件
parse.add_argument('--data_path', type=str, default='./checkpoints/csv/', help='the path for csv files')

#1.2 set gpu
os.environ["CUDA_VASIBLE_DEVICES"] = "0"
...
def train_valid_model(train_dataloader, valid_dataloader, model, loss_function_A, loss_function_B, device, batch_size, inter_log):
    train_epoch_loss, train_epoch_A_loss, train_epoch_B_loss = trainloss.train(train_dataloader, model,
                                                                                loss_function_A, loss_function_B, device, batch_size, inter_log)
    valid_epoch_loss, valid_epoch_A_loss, valid_epoch_B_loss = validloss.valid(valid_dataloader, model,
...
def main():
    ...
    KYD_data = data.read_csv(args.data_path, 'risk_set.csv')
    ...
    train_LFW_data = data.read_csv(args.data_path, 'lfw_train_face_pair.csv')
    valid_LFW_data = data.read_csv(args.data_path, 'lfw_test_face_pair.csv')

    # 4.2.3 read lfw_csv_files for calculate std
    LFW_data_train_std = data.read_csv(args.data_path, 'lfw_train.csv')
    LFW_data_valid_std = data.read_csv(args.data_path, 'lfw_test.csv')

    # 4.2.4 read res_csv_files for gini
    res = data.read_csv(args.data_path, 'Res_facepicturename.csv')
    ...
    KYD_valid_list = data.kyd_merge_list(KYD_valx1, KYD_valx2, KYD_valy)

    # 4.3.2 LFW valid_dataloader
    LFW_valx1, LFW_valx2, LFW_valy = valid_LFW_data.iloc[:, 0], valid_LFW_data.iloc[:, 1], valid_LFW_data.iloc[:, 2]
    LFW_valid_list = data.lfw_merge_list(LFW_valx1, LFW_valx2, LFW_valy)
    ...
    model = models.Backbone(num_layers=50, drop_ratio=0.6, mode='ir')
    ...
    for epoch in range(1, args.epoches + 1):
        ...
        KYD_train_list = data.kyd_merge_list(KYD_trainx1, KYD_trainx2, KYD_trainy)

        # 4.7.2 LFW train_dataloader
        train_LFW_data = train_LFW_data.sample(frac=0.4)
        LFW_trainx1, LFW_trainx2, LFW_trainy = train_LFW_data.iloc[:, 0], train_LFW_data.iloc[:,
                                                                          1], train_LFW_data.iloc[:, 2]
        LFW_valid_list = data.lfw_merge_list(LFW_valx1, LFW_valx2, LFW_valy)
        ...
        valid_epoch_B_loss = train_valid_model(train_loader, valid_loader, model, loss_function_A, loss_function_B,
                                               args.device, args.batch_size, args.inter_log)
        torch.save(model.state_dict(), './' + 'mse_bce_' + str(epoch) + '.pth')

        # 4.7.6 evalute kyd_lfw dataloader, get(name, score)
        KYD_train_output_lists = compute_gini.evalute(KYD_train_loader, model, args.device)
        KYD_valid_output_lists = compute_gini.evalute(KYD_valid_loader, model, args.device)
        LFW_train_output_lists = compute_gini.evalute(LFW_train_loader, model, args.device)
        LFW_valid_output_lists = compute_gini.evalute(LFW_valid_loader, model, args.device)

        # 4.7.7 calculate kyd_score and lfw_score
        KYD_train_score_dict = data.kyd_train_and_valid_score(KYD_train_output_lists, res)
        KYD_valid_score_dict = data.kyd_train_and_valid_score(KYD_valid_output_lists, res)
        LFW_train_output_dict = data.mapping(LFW_train_output_lists, num=100)
        LFW_valid_output_dict = data.mapping(LFW_valid_output_lists, num=100)
        LFW_train_score_dict = data.lfw_train_and_valid_score(LFW_train_output_lists,
                                                                  LFW_train_output_dict['values'],
                                                                  LFW_train_output_dict['aiCentile'])
        LFW_valid_score_dict = data.lfw_train_and_valid_score(LFW_valid_output_dict,
                                                                  LFW_valid_output_dict['values'],
                                                                  LFW_valid_output_dict['aiCentile'])

        # 4.7.8 calculate gini
        train_gini = gini.gini(dataset=KYD_train_score_dict, pred='FACE_SCORE', income='ex_ncd_prem_base_com',
                               claims='inc_com')
        valid_gini = gini.gini(dataset=KYD_valid_score_dict, pred='FACE_SCORE', income='ex_ncd_prem_base_com',
                               claims='inc_com')

        # 4.7.9 calculate std
        _, _, train_std = data.compute_std(LFW_train_score_dict)
        _, _, valid_std = data.compute_std(LFW_valid_score_dict)
        ...
        print_with_time('train_epoch:{}, train_epoch_loss:{}, valid_epoch_loss:{}, train_epoch_margin_loss:{}, '
                        ...
                        'valid_gini:{}, train_std:{}, valid_std:{},'.format(epoch, train_epoch_loss, valid_epoch_loss,
                                                                            train_epoch_A_loss, valid_epoch_A_loss,
                                                                            train_epoch_B_loss, valid_epoch_B_loss,
                                                                            train_gini,
